                    ...
                    <div class="sf-table-wrapper">
                        ...
                        >
                            ...
                            <template #slot-edit>
                                <span class="table-edit">
                                    ...
                                    <el-button
                                        ...
                                        size="small"
                                        :disabled="!chooseRow"
                                        @click="handleViewDetailBtn(chooseRow)"
                                        >查看详情</el-button
                                    >
                                </span>
                            </template>
                            ...
                            <template #col-status="{ row }">
                                <div :style="{ color: row.status === 2 ? '#ccc' : '' }">
                                    {{ getNameById(readStatusList, row.status) }}
                                </div>
                            </template>
...
import { getNameById, noticeClassificationList, readStatusList } from "@/assets/common/select-list"
...
import { getChangeNoticeList, changeNoticeMarkRead } from "@/api/changeManage/initiateChangeNotice"
...
// 标记已读按钮
const handleBatchMarkReadBtn = async () => {
    ...
    BatchMarkReadHandler(oids)
}

const BatchMarkReadHandler = async (oids: string) => {
    try {
        const params = {
            oidsStr: oids
        }
        await changeNoticeMarkRead(params)
