            ...
            <el-row>
              ...
              <el-form-item class="edit-btn">
                ...
                <el-button
                  type="primary"
                  size="small"
                  @click.stop="setOrder('up', chooseRow, tableData)"
                >上移</el-button>
                <el-button
                  type="primary"
                  size="small"
                  @click.stop="setOrder('down', chooseRow, tableData)"
                >下移</el-button>
                <el-button
                  type="primary"
                  size="small"
                  @click.stop="setOrder('top', chooseRow, tableData)"
                >置顶</el-button>
                <el-button
                  type="primary"
                  size="small"
                  @click.stop="setOrder('bottom', chooseRow, tableData)"
                >置底</el-button>
              </el-form-item>
            </el-row>
            <el-row>
              <el-form-item label="特征值" prop="choiceList">
...
import { getOptionListOfPart } from "@/api/configureManage/productConfigure";
...
const rules = reactive<FormRules>({
  ...
  choiceList: [{ required: true, message: '请选择特征值！', trigger: 'change' }],
...
const handleSelectChange_option = (item: any) => {
  // console.log('handleSelectChange_option', item.choiceList);
  formData.choiceList = []
  radio.value = ''
...
if (!props.onlyRead) getList_option()
// 运算符选择回调
...
const goDel = (index: number) => {
  ElMessageBox.confirm('是否确认删除该表达式？', t("dashboardPage.messageBoxTitle"), {
...
// 设置顺序
const setOrder = (type: string, row: any, list: any[]) => {
  if (!chooseRow.value) {
...
const changeType = (type: 'add' | 'edit', row?: any) => {
  ...
  if (type === 'add') {
    ...
    // 重置特征值列表
    for (let i = 0; i < options_option.value.length; i++) {
      const option = options_option.value[i];
