                ...
                <div class="mb-2 flex">
                    ...
                    <template v-if="!isCanEdit">
                        <el-button
                            ...
                            size="small"
                            type="primary"
                            @click="handleDistributionAuthBtn"
...
const getTreeList = async () => {
    ...
    try {
        ...
        defaultExpandRowKeys.value = data.map((item: any) => item.roleOid)
        loading.value = false
    } catch (error: any) {
        console.log(error)
        loading.value = false
...
const handleNodeClick = (node: any) => {
    console.log("角色节点node: ", node)
    if (node.level === "0") {
        roleFormRef.value!.handleClear()
        return
    }
    if (isCanEdit.value) {
        ...
        roleTreeRef.value!.setCurrentKey(currentTreeNodeData.value.roleOid)
        return
    }
    currentTreeNodeData.value = node
    roleFormRef.value!.getDetail(node)
}
...
const handleAddModifyBtn = (type: string) => {
    console.log("roleFormRef: ", roleFormRef.value)
...
const handleSaveBtn = (leaveParams: any) => {
    roleFormRef.value!.handleSaveBtn(leaveParams)
}
// 删除
const handleDeleteBtn = () => {
    roleFormRef.value!.handleDeleteBtn()
}
// 分配权限
const handleDistributionAuthBtn = () => {
    roleFormRef.value!.handleDistributionAuthBtn()
}
